...
namespace STP3D {
	...
	*/
	...
	inline GLuint ShaderManager::loadShader(const char *vertexFile, const char *fragmentFile, bool v) {
		...
		if(v) std::cout << "Initialization of the shader program 1 "<< std::endl;
		programObject = glCreateProgram();
		if(v) std::cout << "Initialization of the shader program 2 "<< std::endl;
		CHECK_GL;
		if(v) std::cout << "Initialization of the shader program 3 "<< std::endl;
		if(!programObject) {
			if(v) {
