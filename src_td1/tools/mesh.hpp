...
namespace STP3D {
	...
	public:
		...
		StandardMesh(uint elts = 0,uint new_gl_type = GL_TRIANGLES) : id_vao(0) {
			buffers.clear();
			size_one_elt.clear();
			attr_id.clear();
			attr_semantic.clear();
		...
		void addOneBuffer(uint id_attribute,uint one_elt_size,
		                  float* data,const std::string& semantic="",bool copy=false);
	...
	inline void StandardMesh::addOneBuffer(uint id_attribute,uint one_elt_size,
	                                       float* data,const std::string& semantic,bool copy) {
		if (copy) {
