...
namespace STP3D {
	...
	public:
		...
		/** Copy constructor
		 * Copy constructor without copying :
		 * - OpenGL index
		 * - Last active texture unit
		 * If \a copy is true (default value) then data from the source texture is copied
		 * \param texsource Texture source
		 * \param copy True by default. Copy the image of the texture
		*/
		Texture2D(const Texture2D* texsource,bool copy=true);
	...
	inline Texture2D::Texture2D(const Texture2D* texsource,bool copie) {
		int decal=-1;

		last_tex_unit = GL_TEXTURE0;
		gl_id_tex = 0;
		typetext = texsource->typetext;
		tex_w = texsource->tex_w;
		tex_h = texsource->tex_h;
		switch (typetext) {
			case (TEX_TYPE_NULL) :
				decal = 0;
				tabRVB = NULL;
			...
			case (TEX_TYPE_RVBA) :
				...
				break;
			default :
				STP3D::setError("[Texture : constructor] Src texture has unknown ("+intToString(typetext)+") texture type when using texture copy constructor");
		}
		if (decal > 0 && copie) {
			tabRVB = new unsigned char[tex_w*tex_h*decal];
			memcpy(tabRVB,texsource->getTab(),tex_w*tex_h*decal);
		}
		else {
			tabRVB = NULL;
		}
	...
	inline bool Texture2D::readTGA(FILE *file) {
		...
		unsigned short* pt = (unsigned short*) (tgaheader.Header+12);
		tex_w = *pt;
		pt = (unsigned short*) (tgaheader.Header+14);
		tex_h = *pt;
